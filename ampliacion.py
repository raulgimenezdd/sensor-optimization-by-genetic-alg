...
def create_individual():  # this function creates a random individual as a binary list with length of n_genes
    ...
    posibilities = ['0', 'H', 'F']
    individual = [random.choice(posibilities) for i in range(n_genes)]
...
def evaluate_population(population):
    ...
    for i in range(len(population)):
        str_individual = "".join(
            [str(_) for _ in population[i]])  # we transform the list that represents an individual into a string
        fitness_individual = http_requests_fitness(str_individual)
        fitness_list.append(fitness_individual)
    return fitness_list


def selection_tournaments(evaluated_population, fitness_list,
                          n_participants):  # this funtion selects the best individuals through tournaments
    print("Selecting population...")
    selected_population = []
    for i in range(
            len(fitness_list)):  # as many iterations (tournaments) as individuals to create a new population of the same size
        selected_individual = -1
        fitness_selected = 100000
        for j in range(n_participants):
            participant = np.random.randint(0,
                                            len(fitness_list))  # random selection of an individual from the whole population
...
def individuals_simple_crossing(
        selected_population):  # this function mix the genes of the individuals simulating the reproduction
    # print("Crossing population...")
    cross_population = []
    for i in range(0, n_individuals, 2):
        crossed_individual1 = selected_population[i][:192] + selected_population[i + 1][
                                                             192:]  # first_part_ind1 + second_part_ind2
        crossed_individual2 = selected_population[i + 1][:192] + selected_population[i][
                                                                 192:]  # first_part_ind2 + second_part_ind1
